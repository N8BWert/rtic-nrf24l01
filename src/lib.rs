...
#[cfg(all(feature = "embedded-hal", feature = "embedded-hal-02"))]
compile_error!("Please choose a version of the embedded-hal to use");
...
pub mod config;
use config::{RegisterValue, address_width::AddressWidth};
...
#[cfg(feature = "embedded-hal")]
impl<CE, CSN, SPI, GPIOE, SPIE> Radio<CE, CSN, SPI, GPIOE, SPIE> where 
    CE: OutputPin<Error=GPIOE>,
    CSN: OutputPin<Error=GPIOE>,
    SPI: SpiDevice<Error=SPIE>,
    GPIOE: Debug,
    SPIE: Debug,
{
    ...
    pub fn write(&mut self, buffer: &[u8], spi: &mut SPI, delay: &mut impl DelayNs) -> bool {
    ...
    pub fn start_fast_write(&mut self, buffer: &[u8], multicast: bool, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn write_payload(&mut self, buffer: &[u8], write_command: Command, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn read(&mut self, buffer: &mut [u8], spi: &mut SPI, delay: &mut impl DelayNs) {
        self.read_payload(buffer, spi, delay);

        self.write_byte_register(Register::Status, bit_value(RX_DR), spi, delay);
    }

    pub fn read_payload(&mut self, buffer: &mut [u8], spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn receive_packet(&mut self, buffer: &mut [u8], spi: &mut SPI, delay: &mut impl DelayNs) -> Result<u8, RadioError> {
    ...
    pub fn packet_ready(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> bool {
        let status = self.read_status(spi, delay);
        if status & (1 << 6) != 0 {
    ...
    pub fn set_address_width(&mut self, address_width: AddressWidth, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn power_up(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn power_down(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn start_listening(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn stop_listening(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
        let _ = self.ce.set_low();
    ...
    pub fn open_writing_pipe(&mut self, address: [u8; 5], spi: &mut SPI, delay: &mut impl DelayNs) {
        self.write_register(Register::RxAddressP0, &address, spi, delay);
        self.write_register(Register::TxAddress, &address, spi, delay);
    }

    pub fn set_payload_size(&mut self, size: u8, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn set_retries(&mut self, delay: u8, count: u8, spi: &mut SPI, d: &mut impl DelayNs) {
        self.write_byte_register(Register::SetupRetransmit, 
            min(15, delay) << 4 | min(15, count), spi, d);
    }

    pub fn open_reading_pipe(&mut self, pipe: u8, address: [u8; 5], spi: &mut SPI, delay: &mut impl DelayNs) {
        if pipe == 0 {
            self.is_p0_rx = true;
        } else if pipe <= 5{
            if pipe < 2 {
    ...
    pub fn close_reading_pipe(&mut self, pipe: u8, spi: &mut SPI, delay: &mut impl DelayNs) {
        let open_pipes = self.read_byte_register(Register::EnableRx, spi, delay) & !bit_value(pipe);
    ...
    pub fn begin(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> Result<(), RadioError> {
        self.init_pins(delay);
        self.init_radio(spi, delay)
    }

    fn init_pins(&mut self, delay: &mut impl DelayNs) {
        let _ = self.ce.set_low();
        let _ = self.csn.set_high();
        delay.delay_ms(100);
    }

    fn init_radio(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> Result<(), RadioError> {
        delay.delay_ms(5);
    ...
    pub fn toggle_features(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
        let mut buffer = [Command::Activate.opcode()];
        self.safe_transfer_spi(&mut buffer, spi, delay);
        self.status = buffer[0];
    }

    pub fn available(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> bool {
        let fifo_status = self.read_byte_register(Register::FifoStatus, spi, delay);

        return fifo_status & 1 == 0;
    }

    pub fn available_pipe(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> u8 {
    ...
    pub fn reset(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn flush_tx(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
        let mut command = [Command::FlushTx.opcode(), 0xFF];
        self.safe_transfer_spi(&mut command, spi, delay);
    }

    pub fn flush_rx(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
        let mut command = [Command::FlushRx.opcode(), 0xFF];
        self.safe_transfer_spi(&mut command, spi, delay);
    }

    pub fn read_config(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> u8 {
        self.read_byte_register(Register::Config, spi, delay)
    }

    pub fn read_status(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> u8 {
    ...
    pub fn read_fifo_status(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> u8 {
        self.read_byte_register(Register::FifoStatus, spi, delay)
    }

    pub fn clear_interrupts(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) {
        self.write_byte_register(Register::Status, 0xFF, spi, delay);
    }

    pub fn is_rx(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> bool {
    ...
    pub fn is_tx(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> bool {
        let config = self.read_config(spi, delay);
        if config & 0b11 == 0b10 {
            true
        } else {
    ...
    pub fn pulse_ce(&mut self, delay: &mut impl DelayNs) {
        let _ = self.ce.set_high();
        delay.delay_ms(1000);
        let _ = self.ce.set_low();
    }

    pub fn set_pa_level(&mut self, level: power_amplifier::PowerAmplifier, spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn set_channel(&mut self, channel: u8, spi: &mut SPI, delay: &mut impl DelayNs) {
        self.write_byte_register(Register::RfChannel, min(channel, 125), spi, delay);
    }

    pub fn set_data_rate(&mut self, data_rate: DataRate, spi: &mut SPI, delay: &mut impl DelayNs) -> bool {
    ...
    pub fn get_registers(&mut self, spi: &mut SPI, delay: &mut impl DelayNs) -> RegisterMap {
    ...
    pub fn write_byte_register(&mut self, register: Register, data: u8, spi: &mut SPI, delay: &mut impl DelayNs) {
        let mut command = [Command::WriteRegister(register).opcode(), data];
        self.safe_transfer_spi(&mut command, spi, delay);
        self.status = command[0];
    }

    fn write_register(&mut self, register: Register, data: &[u8], spi: &mut SPI, delay: &mut impl DelayNs) {
    ...
    pub fn read_byte_register(&mut self, register: Register, spi: &mut SPI, delay: &mut impl DelayNs) -> u8 {
    ...
    pub fn read_register(&mut self, register: Register, buffer: &mut [u8], spi: &mut SPI, delay: &mut impl DelayNs) {
        let buffer_length = buffer.len();
    ...
    fn safe_transfer_spi(&mut self, data: &mut [u8], spi: &mut SPI, delay: &mut impl DelayNs) {
        let _ = self.csn.set_low();
        let _ = spi.transfer_in_place(data);
        let _ = self.csn.set_high();
        delay.delay_us(10);
    }
}
...
#[cfg(feature = "embedded-hal-02")]
impl<CE, CSN, SPI, GPIOE, SPIE> Radio<CE, CSN, SPI, GPIOE, SPIE>
    ...
    pub fn write(&mut self, buffer: &[u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> bool {
        self.start_fast_write(buffer, false, spi, delay);
    ...
    pub fn start_fast_write(&mut self, buffer: &[u8], multicast: bool, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.write_payload(buffer, if multicast { Command::WriteTxPayloadNoAck } else { Command::WriteTxPayload }, spi, delay);
        let _ = self.ce.set_high();
    }

    pub fn write_payload(&mut self, buffer: &[u8], write_command: Command, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut write_buffer = [0xFF; 33];
        write_buffer[0] = write_command.opcode();
        write_buffer[1..=buffer.len()].copy_from_slice(&buffer[..]);
        self.safe_transfer_spi(&mut write_buffer[..=buffer.len()], spi, delay);
    ...
    pub fn read(&mut self, buffer: &mut [u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.read_payload(buffer, spi, delay);

        self.write_byte_register(Register::Status, bit_value(RX_DR), spi, delay);
    }

    pub fn read_payload(&mut self, buffer: &mut [u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut read_buffer = [0xFF; 33];
        read_buffer[0] = Command::ReadRxPayload.opcode();
        self.safe_transfer_spi(&mut read_buffer[..=buffer.len()], spi, delay);
        for i in 0..buffer.len() {
    ...
    /// Receive a packet from the Rx Fifo, returning the pipe it is from
    pub fn receive_packet(&mut self, buffer: &mut [u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> Result<u8, RadioError> {
        if !self.packet_ready(spi, delay) {
            return Err(RadioError::NoPacketReady);
        }
    ...
    /// Check whether there is a packet ready
    pub fn packet_ready(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> bool {
        let status = self.read_status(spi, delay);
        if status & (1 << 6) != 0 {
            self.clear_interrupts(spi, delay);
            true
    ...
    /// Set the length of the radio's address width
    pub fn set_address_width(&mut self, address_width: AddressWidth, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.write_byte_register(Register::SetupAddressWidth, address_width.register_value(Register::SetupAddressWidth), spi, delay);
        self.address_width = address_width.as_usize() as u8;
    }

    /// Put the radio into a power up mode.
    pub fn power_up(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        if (self.config_register & bit_value(PWR_UP)) == 0 {
            self.config_register |= bit_value(PWR_UP);
            self.write_byte_register(Register::Config, self.config_register, spi, delay);
            delay.delay_ms(START_UP_US);
    ...
    /// Put the radio into a powered down mode.
    pub fn power_down(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut config = self.read_config(spi, delay);
        config &= !(1 << 1);
        self.write_byte_register(Register::Config, config, spi, delay);
    ...
    /// Put the radio into RX mode an listen for incoming packets
    pub fn start_listening(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.power_up(spi, delay);
    ...
    pub fn stop_listening(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let _ = self.ce.set_low();
    ...
    /// Open the previously set writing pipe
    pub fn open_writing_pipe(&mut self, address: [u8; 5], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.write_register(Register::RxAddressP0, &address, spi, delay);
        self.write_register(Register::TxAddress, &address, spi, delay);
    }

    pub fn set_payload_size(&mut self, size: u8, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.payload_size = max(1, min(32, size));
    ...
    pub fn set_retries(&mut self, delay: u8, count: u8, spi: &mut SPI, d: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.write_byte_register(Register::SetupRetransmit, 
            min(15, delay) << 4 | min(15, count), spi, d);
    }

    pub fn open_reading_pipe(&mut self, pipe: u8, address: [u8; 5], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        if pipe == 0 {
            self.is_p0_rx = true;
        } else if pipe <= 5{
            if pipe < 2 {
    ...
    /// Close a given rx pipe
    pub fn close_reading_pipe(&mut self, pipe: u8, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let open_pipes = self.read_byte_register(Register::EnableRx, spi, delay) & !bit_value(pipe);
        self.write_byte_register(Register::EnableRx, open_pipes, spi, delay);
        if pipe == 0 {
            self.is_p0_rx = false;
    ...
    /// Apply the configuration given to the driver and return true if the radio is configured correctly.
    pub fn begin(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> Result<(), RadioError> {
        self.init_pins(delay);
        self.init_radio(spi, delay)
    }

    fn init_pins(&mut self, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let _ = self.ce.set_low();
        let _ = self.csn.set_high();
        delay.delay_ms(100);
    }

    fn init_radio(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> Result<(), RadioError> {
        delay.delay_ms(5);
    ...
    pub fn toggle_features(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut buffer = [Command::Activate.opcode()];
        self.safe_transfer_spi(&mut buffer, spi, delay);
        self.status = buffer[0];
    }

    pub fn available(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> bool {
        let fifo_status = self.read_byte_register(Register::FifoStatus, spi, delay);

        return fifo_status & 1 == 0;
    }

    pub fn available_pipe(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> u8 {
        let pipe = (self.read_status(spi, delay) >> 1) & 0x07;
        pipe
    }
    ...
    /// Reset the radio to it's default state (theoretically, this should be called every time the radio is
    /// so that a defined state is available.  However, begin should (in theory) do the same thing).
    pub fn reset(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        delay.delay_ms(POWER_ON_RESET_MS);
    ...
    pub fn flush_tx(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut command = [Command::FlushTx.opcode(), 0xFF];
        self.safe_transfer_spi(&mut command, spi, delay);
    }

    pub fn flush_rx(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut command = [Command::FlushRx.opcode(), 0xFF];
        self.safe_transfer_spi(&mut command, spi, delay);
    }

    pub fn read_config(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> u8 {
        self.read_byte_register(Register::Config, spi, delay)
    }

    pub fn read_status(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> u8 {
        let mut command = [Command::Nop.opcode()];
        self.safe_transfer_spi(&mut command, spi, delay);
        self.status = command[0];
        self.status
    ...
    pub fn read_fifo_status(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> u8 {
        self.read_byte_register(Register::FifoStatus, spi, delay)
    }

    pub fn clear_interrupts(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.write_byte_register(Register::Status, 0xFF, spi, delay);
    }

    pub fn is_rx(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> bool {
        let config = self.read_config(spi, delay);
        if config & 0b11 == 0b11{
            true
        } else {
    ...
    pub fn is_tx(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> bool {
        let config = self.read_config(spi, delay);
        if config & 0b11 == 0b10 {
            true
        } else {
    ...
    pub fn pulse_ce(&mut self, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let _ = self.ce.set_high();
        delay.delay_ms(1000);
        let _ = self.ce.set_low();
    }

    pub fn set_pa_level(&mut self, level: power_amplifier::PowerAmplifier, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut setup = self.read_byte_register(Register::RfSetup, spi, delay) & 0xF8;
        setup |= level.register_value(Register::RfSetup);
        self.write_byte_register(Register::RfSetup, setup, spi, delay);
    }

    pub fn set_channel(&mut self, channel: u8, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        self.write_byte_register(Register::RfChannel, min(channel, 125), spi, delay);
    }

    pub fn set_data_rate(&mut self, data_rate: DataRate, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> bool {
        let mut result = false;
        let mut setup = self.read_byte_register(Register::RfSetup, spi, delay);
    ...
    // Debugging function that returns a debug printable mapping of the device's internal registers
    pub fn get_registers(&mut self, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> RegisterMap {
        let mut register_map = RegisterMap::default();
    ...
    pub fn write_byte_register(&mut self, register: Register, data: u8, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut command = [Command::WriteRegister(register).opcode(), data];
        self.safe_transfer_spi(&mut command, spi, delay);
        self.status = command[0];
    }

    fn write_register(&mut self, register: Register, data: &[u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let mut command = [0xFFu8; 6];
        command[0] = Command::WriteRegister(register).opcode();
        command[1..=data.len()].copy_from_slice(&data[..]);
        self.status = command[0];
    ...
    pub fn read_byte_register(&mut self, register: Register, spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) -> u8 {
        let mut command = [Command::ReadRegister(register).opcode(), 0xFF];
        self.safe_transfer_spi(&mut command, spi, delay);
        self.status = command[0];
        command[1]
    ...
    pub fn read_register(&mut self, register: Register, buffer: &mut [u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let buffer_length = buffer.len();
        let mut command = [0xFF; 6];
        command[0] = Command::ReadRegister(register).opcode();
        self.safe_transfer_spi(&mut command, spi, delay);
    ...
    fn safe_transfer_spi(&mut self, data: &mut [u8], spi: &mut SPI, delay: &mut (impl DelayMs<u32> + DelayUs<u32>)) {
        let _ = self.csn.set_low();
